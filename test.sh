...
echo $OS_TOKEN
export PUBLIC_create=$(curl -s http://127.0.0.1:9696/v2.0/networks -X POST -H "Content-Type: application/json" -H "Accept: application/json" -H "X-Auth-Token:$OS_TOKEN" -d '{"network": {"name":"PUBLIC","provider:network_type":"vlan","provider:physical_network":"public","shared":"true","router:external":"true"}}' | python -m json.tool > p.json)
